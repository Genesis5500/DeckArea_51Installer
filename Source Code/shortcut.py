...
def shortcut_exists():
    try:
        ...
        return False
def create_shortcut():
    print("Creating Steam shortcut for Area 51")
    shortcut_path = get_shortcuts_vdf_path()
...
def generate_preliminary_id():
    exe = f'"{Path.home()}/project_dreamland/resources/app/game/A51.exe"'
    unique_id = ''.join([exe])
...
def generate_shortcut():
    shutil.which(str(Path.home()) + "/project_dreamland/resources/app/game/A51.exe")

    return {
        'appid': generate_shortcut_id(),
        'AppName': 'Area 51 (2005) (Project Dreamland)',
        'Exe': f'"{str(Path.home())}/project_dreamland/resources/app/game/A51.exe"',
        'StartDir': f'"{str(Path.home())}/project_dreamland/resources/app/game/"',
        'LaunchOptions': '--launcher',
        'IsHidden': 0,
